...
const StudentsSection = ({
  ...
  const handleUpdateStudent = async () => {
    try {
      const coordinatorToken= localStorage.getItem("coordinatorToken"); // Get the schoolToken from localStorage
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_HOSTNAME}/api/gio/update-profile`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${coordinatorToken}`, // Add the token from localStorage
          },
      ...
      <div className="w-[90%] max-w-6xl mx-auto bg-white rounded-2xl shadow-lg">
        ...
        <div className="w-[90%] max-w-6xl mx-auto mt-5 bg-white rounded-2xl shadow-lg">
          <div className="w-full px-4 py-4">
            <div className="grid grid-cols-1 md:grid-cols-9 gap-x-4 gap-y-4 text-gray-700 font-semibold text-sm border-b pb-3">
              {/* Name Filter */}
            ...
            {displayedStudents.map((student, index) => {
              ...
              return (
                <div
                  key={index}
                  className={`grid grid-cols-1 md:grid-cols-9 gap-x-3 text-gray-800 text-sm py-3 border-b last:border-none relative ${
